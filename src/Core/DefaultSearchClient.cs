...
public class DefaultSearchClient : ISearchClient
{
    ...
    protected readonly SearchClientConfig _config;
    protected readonly ILogger<DefaultSearchClient> _log;
    protected IPromptProvider _promptProvider;
    protected string _answerPrompt;
    ...
    public DefaultSearchClient(
        ...
        ILogger<DefaultSearchClient>? log = null)
    {
        ...
        {
            throw new KernelMemoryException("Prompt provider not configured");
        }

        _promptProvider = promptProvider;

        this._answerPrompt = promptProvider.ReadPrompt(Constants.PromptNamesAnswerWithFacts);
        this._log = log ?? DefaultLogger<DefaultSearchClient>.Instance;
    }

    /// <inheritdoc />
    public virtual Task<IEnumerable<string>> ListIndexesAsync(CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public virtual async Task<MemoryAnswer> AskAsync(
        string index,
        string question,
        ICollection<MemoryFilter>? filters = null,
        double minRelevance = 0,
    ...
    protected virtual IAsyncEnumerable<string> GenerateAnswerAsync(string question, string facts)
    {
    ...
    protected static bool ValueIsEquivalentTo(string value, string target)
    {
